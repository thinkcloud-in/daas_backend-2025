...
async def get_vms(cluster_id:int,db: Session = Depends(get_db)):
        return await controller.get_vms(cluster_id,db)

@hyper_v_router.get("/ping_agent")
async def ping_agent(
    ...
    return await controller.ping_agent(cluster_id, ip, port, db)
...
async def get_vm_info(vm_id: str, db: Session = Depends(get_db)):
        return await controller.get_vm_info(vm_id, db)
...
async def get_switches(cluster_id:int,db: Session = Depends(get_db)):
        return await controller.get_switches(cluster_id,db)
...
async def delete_vm(vm_id: str, db: Session = Depends(get_db)):
        return await controller.delete_vm(vm_id, db)
...
async def get_status(vm_id: str, db: Session = Depends(get_db)):
        return await controller.get_status(vm_id, db)
...
async def rebuild_vm(request: HandleRebuildActionRequest, db: Session = Depends(get_db)):
        return await controller.rebuild_vm_endpoint(request, db)
...
async def verify_standalone_hyper_v(request: VerifyStandaloneHyperVRequest,cluster_id: Optional[int] = None,    db: Session = Depends(get_db) ):
        return await controller.verify_standalone_hyper_v(request,cluster_id,db)
