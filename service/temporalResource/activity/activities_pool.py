...
# Activity funtion for delete the pool
# @activity.defn()
...
#             # Check if pool_machines has any items
#             if len(pool_machines) > 0:
...
#         else:
#             print("Pool not found with id- {pool_id}")
#             return {"msg": f"Pool not found with id - {pool_id}"}, 404
#     except Exception as e:
#         db.rollback()
#         # print("Pool deletion failed")
...
#     db: Session = next(get_db())
#     pool_data = {key: request[key] for key in CreatePoolBase.__annotations__.keys() if key in request}
#     clone_payload = {key: request[key] for key in ClonePayload.__annotations__.keys() if key in request}
#     email = pool_data.pop("email", None)

#     ip_pool_names = clone_payload.get("ip_pool_names")
#     if not ip_pool_names or not isinstance(ip_pool_names, list) or not ip_pool_names:
#         raise HTTPException(status_code=400, detail="No IP pools selected for automated assignment. Please select at least one IP pool.")

#     vm_count = clone_payload.get("count", 0)
...
#         db.add(pool)
#         db.commit()
#         db.refresh(pool)  # Now pool.id is available
#         id_pool = pool.id
#         id_cluster = clone_payload.get("cluster_id")
#         cluster_pool_id = f"{id_pool}_{id_cluster}"
#         pool.cluster_id = cluster_pool_id
#         db.commit()

#         machines_json = []
#         if pool.pool_type == "Automated" and clone_payload:
#             cluster_id = clone_payload.get("cluster_id")
#             if not cluster_id:
#                 raise HTTPException(status_code=400, detail="Cluster ID is required for automated pool creation.")

#             cluster_data = db.query(Cluster).filter(Cluster.id == cluster_id).first()
#             nodes = clone_payload.get("node")
...
#             assigned_nodes = [nodes[i % len(nodes)] for i in range(num_allocated)]

#             # This call clones, assigns, and saves each VM in DB
#             machines_json = await clone_and_assign_ips_sequentially(
#                 db=db,
#                 pool=pool,
#                 nodes=nodes,
#                 template_vm_id=clone_payload["template_vm_id"],
#                 name_template=clone_payload["name_template"],
...
#                 headers=headers,
#                 base_machine_data={
#                     # Everything except name/vm_id/guacd_hostname
#                     "port": pool.pool_port,
#                     "protocol": pool.pool_protocol,
#                     "guacd_port": pool.pool_guacd_port,
#                     "guacd_encryption": pool.pool_guacd_encryption,
